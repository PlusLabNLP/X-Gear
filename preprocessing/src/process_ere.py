...
nlp_es_dep = stanza.Pipeline(lang='es', tokenize_pretokenized=True)
...
def sentence_tokenize(sentence: Tuple[int, int, str],
    ...
    #         sents.append(text[ending_char_idx[idx-1]: ending_char_idx[idx]])
...
class Entity_whole:
    ...
    def copy(self):
        """Makes a copy of itself.
        """
        return Entity_whole(self.entity_id, self.entity_type, copy.deepcopy(self.entity_mentions))
...
def read_annotation(path: str
    ...
    #if doc_id == 'XIN_SPA_20050402.0105':
    #    ipdb.set_trace()
...
def process_batch(input_dir, output_file, window_size=1, dataset='normal', language='english'):
    ...
    with open(output_file, 'w', encoding='utf-8') as w:
        for source_file in source_files:
            ...
            cnt += 1
    print('Processed {} document!!!!'.format(cnt))
    ...
    if args.lang == 'english':
        ...
        for d_path, dataset in sub_set:
            ...
            oneie_path = os.path.join(args.output, '{}.{}.{}.oneie.json'.format("english",
                                                                            dataset,
                                                                            f_size))
            ere_to_oneie(json_path, oneie_path, language='english', tokenizer=tokenizer)
        ...
        for d_path, dataset in sub_set:
            ...
            oneie_path = os.path.join(args.output, '{}.{}.oneie.json'.format("spanish",
                                                                            f_size))
            ere_to_oneie(json_path, oneie_path, language='spanish', tokenizer=tokenizer)
